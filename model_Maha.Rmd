...
# p1=1-CDF.CHISQ(d-squared,nobsvar) TO GET THE PROBABILITIES
...
df$P = 1 / (1 + exp(-10*(lambda - 0.5)))
  
  # 1 / sqrt(det(2 * pi * cov)) * exp(- D_squared / 2)

# pchisq(D_squared, dim(x)[2])
  

# df$P = 1 - pchisq(df$P, dim(x)[2])
...
writeRaster(Mahalonobis, 'Maha_Dist.tif', overwrite = T)
```
